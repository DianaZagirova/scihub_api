...
class SciHubFastDownloader:
    ...
    def download_and_process(self, identifier):
        ...
        try:
            ...
            if extracted_data:
                logger.info(f"Successfully processed paper with fast parser: {identifier}")
                self.log_entry(self.success_log, identifier, pdf_path)
                return pdf_path, extracted_data, 'success'
            else:
                logger.error(f"Failed to process paper with fast parser: {identifier}")
                self.log_entry(self.processing_failed_log, identifier, f"{pdf_path} - Processing returned no data")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Download papers from Sci-Hub using DOI, PMID, or title and process with fast PDF parser')
